...
// NewNamed is similar to New(...) but can be used to set a customProvider name for the
...
// NewCustomisedURL is similar to New(...) but can be used to set customProvider URLs hence omit the auto-discovery step
...
func (p *Provider) BeginAuth(state string) (goth.Session, error) {
	...
	coderVerifier := oauth2.GenerateVerifier()
	...
	session := &Session{
		AuthURL:      parse.String(),
		CodeVerifier: coderVerifier,
	}
	return session, nil
}
